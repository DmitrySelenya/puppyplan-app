...
**Checklist:**
- [ ] Apply the relevant `CHANGELOG-pass3` point-fixes per screen (FAB policy, onboarding 2.5 wizard-chrome vs 2.6 first-Today split, single-feedback celebration, error-state form matching, a11y button upgrades).
- [ ] Quick Log: one `potty` tile + subtype segment in the details form; `Weight` tile (if present) deep-links to Health (Decision 7).
- [ ] Structural anatomy + a11y tests; side-by-side v2-atlas compare for every changed state.
...
**Checklist:**
- [ ] Dynamic Type XXL/XXXL (EN/RU/ES), VoiceOver/TalkBack, screen-states matrix parity, Maestro flows — re-run against v2.
