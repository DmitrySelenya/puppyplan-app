...
Task 6 regression evidence (2026-07-01): pre-Task-7 screenshot review found one remaining open-state drift: when the capsule unmounted, the Add/Close control was the only remaining child in the centered nav row, so it moved to the horizontal center and covered the `Schedule` slab text. RED `npm run test:unit -- --runTestsByPath src/test/capsule-tab-bar.render.test.tsx` failed on missing stable `nav-capsule-slot`; GREEN kept a non-accessible, token-sized capsule layout slot mounted while removing the actual `nav-capsule`/tabs, and wrapped Add in `nav-add-slot` so the morph remains in place. Targeted nav tests passed 26/26; `npm run typecheck`, `node scripts/checks/text-hygiene.mjs`, and `git diff --check` passed. JS-over-Metro simulator verification on `Grith iPhone SE 3 iOS 26.3` captured `output/v2-nav-capsule-screenshots/04-add-open-fixed.jpg`, showing `×` retained on the right in the original Add position without covering the `Schedule` label.
