...
5. **Guidance completion persistence**
   - **Chosen:** Keep guidance read/practiced/skip transitions typed and in-session for this batch; do not fake durable completion with Zustand or route-local storage.
   - **Reason:** Durable guidance progress needs an explicit server-state or approved local-storage contract. No schema/storage change is approved in this batch.

6. **Dev gallery feature composition**
   - **Chosen:** Treat `src/features/_dev/design-gallery` cross-feature preview imports as a confirmed review follow-up, not as production behavior.
   - **Reason:** Fixing this correctly requires either an explicit architecture exception for dev-only inventory routes or moving gallery composition outside feature ownership. This patch does not change architecture docs without explicit approval.
