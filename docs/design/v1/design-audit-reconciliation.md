...
| `text/tertiary` contrast should be corrected from `#76796F`. | open | Current raw `tokens.css` uses `#72756B`, but canonical root `DESIGN.md` and `design-tokens.json` still list `#76796F`. Resolve in the later token pipeline/design-doc sync work, not by copying raw CSS into native code. |
...
| CSS token coverage gaps: `primary-900`, sheet shadow token usage, snackbar action token, tabular numerals. | open | Current raw CSS has tabular numerals and some tokenized fixes, but `primary-900` is still absent and source/token parity is not guaranteed. Treat as token-pipeline input. |
