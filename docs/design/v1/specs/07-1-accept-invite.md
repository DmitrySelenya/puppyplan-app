...
Allowed deviations: live token lookup, accept RPC, decline RPC, and post-accept redirect remain
deferred. Deterministic loading/error/expired/already-member state templates are synthetic review
states only.
...
- Loading invite, load error, expired/unavailable, and already-member — synthetic review states.
- Real accept/decline pending, live token lookup, expired/revoked provider payloads, and post-accept
  redirect — deferred to data-wiring slice.
