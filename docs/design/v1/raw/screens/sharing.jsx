...
function SimpleHeader({ title, left = 'Cancel', right }) {
  return (
    <div className="pp-navbar" style={{ borderBottom: '1px solid var(--pp-stroke-hair)' }}>
      <span style={{ color: 'var(--pp-text-link)', fontSize: 17 }}>{left}</span>
      <span className="pp-nav-title">{title}</span>
      <span style={{ color: 'var(--pp-text-link)', fontSize: 17, fontWeight: right === 'Save' ? 600 : 400, minWidth: 40, textAlign: 'right' }}>{right}</span>
    </div>
  );
}
