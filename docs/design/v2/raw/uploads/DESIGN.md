...
|---|---|---|
| **Опекун A, solo first-time owner** | плохо спит, перегружена, не потерпит долгий setup | onboarding ≤45 сек, anonymous auth, first-log до account wall |
| **Опекун B и Опекун A, shared care** | боятся duplicate feeding/walks | last-action attribution, duplicate warning, household activity strip |
| **Опекун C, owner + trainer** | хочет показать прогресс без раскрытия всего household | trainer_viewer role, scope selector с included/excluded preview |
...
```
  ...
  | Щенок A спала 6 часов — мочевой полный.   |
  | [ Логировать выход ]                   |
  +----------------------------------------+
```
...
- Hero: «Утренний выход» + «Щенок A спала 6 часов — мочевой полный.»
...
Триггер: tap по tracker, по которому есть event того же типа за последние 12 минут от любого household member.

Threshold: 12 минут — source of truth, синхронизировано с PRD §4 и §3.1.7.
...
- Timestamp: friendly bucket. «только что», «12 минут назад», «12:40», «вчера 18:15».
...
```
+----------------------------------------------------------+
|              Вас приглашает Дмитрий                      |
|         присоединиться к заботе о Щенок Bе                |
|                                                          |
|  Ваша роль: Опекун                                       |
|                                                          |
|  +----------------------------------------------------+  |
...
|  +----------------------------------------------------+  |
|                                                          |
|  Дмитрий сможет в любой момент закрыть вам доступ.       |
|                                                          |
|  [ Принять ]                                             |
|  [ Отклонить ]                                           |
+----------------------------------------------------------+
...
- *expired* — neutral state «Это приглашение больше недоступно. Попросите Дмитрия отправить новое.»
- *revoked* — тот же neutral state, формулировка идентична (мы не раскрываем, отозвано или истекло).
- *already-member* — «Вы уже в household Щенка B.» + кнопка «Открыть».
...
|  Приглашения                                             |
|  +----------------------------------------------------+  |
|  | caregiver-c@example.test                  Ожидает       ... | |
|  |  Действует до 24 мая                                | |
|  +----------------------------------------------------+  |
|                                                          |
|  [ Пригласить ]                                          |
...
**Цель.** Поймать случай, когда два опекуна параллельно логируют одно и то же кормление/туалет в 12-минутном окне.
...
- *multiple matches* — список до 2 событий + «Ещё 1 событие за последние 12 минут».
...
- Walk: «Дмитрий вернулся с прогулки — 1 ч назад»
- Reminder created: «Опекун A добавила напоминание — только что»
- Note: «Дмитрий оставил заметку — 30 мин назад»
- Health log: «Дмитрий записал визит к ветеринару — 2 ч назад»
...
> **V2 update (2026-06-23, ADR-0018 + `docs/design/v2/specs/sharing-sitter-trainer-v2.md`):** этот экран сливается с приглашением в **один шаг** — действие «Поделиться с пэтситтером» собирает человека + окно + чеклист и отправляет invite-ссылку (`/invite/*`). Состояние `no-caregiver` («сначала пригласите опекуна») удалено. Щенок появляется у пэтситтера в его же приложении, помеченным бейджем, рядом с его собственными щенками (один аккаунт — много доступов). Богатая модель (окно/чеклист/completion-push/авто-истечение) сохраняется.
...
- Completion success in-app: после tap checklist item — inline label «Готово. Дмитрий получил уведомление.»
...
> **V2 update (2026-06-23, ADR-0018):** первичное действие — **«Создать ссылку» / «Скопировать ссылку»**; отправка на email демотирована в опциональное поле. После создания — карточка ссылки с Copy / Share / Revoke / срок (переиспользует §3.3.4).
...
> **V2 update (2026-06-23, ADR-0018):** этот экран рендерится как **веб-страница в браузере** (`WebFrame`), а не внутри приложения — у тренера нет ни аккаунта, ни установленного приложения. Страница **живая** (перезапрос проекции при каждом открытии) и явно сообщает «обновляется по мере того, как владелец записывает активность» и «аккаунт не нужен». ASCII-разметка ниже остаётся корректной по содержанию — меняется только контекст рендера.

```
+----------------------------------------------------------+
|  {Имя}                                                   |
|  4 месяца • доступ от Дмитрия                            |
|                                                          |
...
> **V2 update (2026-06-23, ADR-0018):** для тренера и получателя card-ссылки этот нейтральный экран — тоже **веб-страница** (`WebShareUnavailable` в `WebFrame`). In-app вариант остаётся для отозванного членства внутри приложения.
...
**Состояния.**
- completed: «Готово · 19:24 · Опекун A». Push «trusted-sitter completion» уходит владельцу.
...
- [ ] Time formatting: «12 минут назад» / «Вчера, 21:10» / «12 мая, 09:42»
