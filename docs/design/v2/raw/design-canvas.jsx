...
function DCViewport({ children, minScale = 0.1, maxScale = 8, style = {} }) {
  ...
  React.useLayoutEffect(() => {
    ...
    // Sanity guard: a persisted pan can strand the view in empty space (the
    // unloading page flushes its in-memory transform on pagehide, so a bad
    // value survives reloads). If the restored/default view leaves no content
    // on screen, fit the whole canvas to the viewport so it can't get stuck.
    ...
    setTimeout(() => {
      ...
      const M = 48; // require at least this many px of content on screen
      const offscreen =
        wr.right < vr.left + M || wr.left > vr.right - M ||
        wr.bottom < vr.top + M || wr.top > vr.bottom - M;
      if (!offscreen) return;
      const cw = world.offsetWidth || 1, ch = world.offsetHeight || 1;
      const fit = Math.min((vr.width - 120) / cw, (vr.height - 120) / ch);
      tf.current = { x: 60, y: 60, scale: Math.min(maxScale, Math.max(minScale, fit)) };
      apply();
    }, 120);
