...
function Phone({ children, scrim = false, style }) {
  return (
    <div className="pp-app pp-phone" style={style}>
      ...
      {scrim && <div role="button" aria-label="Close" tabIndex={0} style={{ position: 'absolute', inset: 0, background: 'rgba(40,30,22,0.34)', cursor: 'pointer' }} />}
    </div>
  );
}
...
// TextLink — tertiary action. Rendered as a real <button> (a11y commit 4):
// role, focus ring, keyboard, and a 44pt hit area via padding compensated with
// negative margins so surrounding layout doesn't shift.
function TextLink({ children, color, onClick, style }) {
  return (
    <button
      type="button"
      ...
      style={{
        ...
        fontFamily: 'var(--pp-font)', lineHeight: '22px',
        color: color || 'var(--pp-text-link)', fontSize: 17, fontWeight: 400,
...
// NavbarAction — navbar text/icon action (“Cancel”, “Save”, back chevron, bell, add).
// Real <button>, min 44×44pt tap target, focus ring. A11y commit 4.1–4.2.
function NavbarAction({ children, onClick, ariaLabel, bold, align, style }) {
  return (
    <button
      type="button"
      onClick={onClick}
      ...
      style={{
        ...
        background: 'transparent', border: 0, padding: '0 8px', margin: '-10px -8px',
        color: 'var(--pp-text-link)', fontSize: 17, fontWeight: bold ? 600 : 400,
...
const PILL_PRESETS = {
  ...
  // Key matches STRINGS.en.json / design-tokens.json `needs-vet-review` (audit commit 2.1).
...
// Pass role="radio" + selected for single-choice rows (invite role pick, sitter “Who”) — audit 4.4.
function ListRow({ leading, title, subtitle, meta, trailing, chevron, onClick, danger, style, ariaLabel, role: roleProp, selected }) {
  const interactive = !!(onClick || chevron || roleProp);
  const a11y = interactive
    ? {
        role: roleProp || 'button',
        tabIndex: 0,
        'aria-label': ariaLabel || [title, subtitle].filter(Boolean).join('. '),
        'aria-checked': (roleProp === 'radio' || roleProp === 'checkbox') ? !!selected : undefined,
  ...
  return (
    <>
      ...
      <div className="pp-tabbar-safe">
        <div style={{ width: 134, height: 5, borderRadius: 5, background: 'var(--pp-text-primary)', margin: '24px auto 0' }} />
      </div>
    </>
  );
}
...
// FAB POLICY (review pass 3, P0): the FAB lives ONLY on log surfaces — Today,
// Timeline, Health. Never on Reminders, More, Settings, or read-only views.
// While a snackbar is visible the FAB is hidden (snackbar's Undo must never sit
// under it). Scroll lists on FAB screens reserve a bottom inset (see tokens.css).
    ...
    <div style={{
      ...
      <button
        ...
        style={{
          ...
          display: 'flex', alignItems: 'center', justifyContent: 'center',
          boxShadow: '0 6px 16px rgba(70,50,30,0.18), 0 0 0 1px rgba(0,0,0,0.04)',
...
// Snackbar
// ────────────────────────────────────────────────────────────────
// Snackbar — host rule: while a snackbar is visible, the FAB is hidden (P0 fix:
// Undo used to sit exactly under the FAB on 4.2 / 2.6). Actions are real buttons.
function Snackbar({ children, action, secondary, onAction, style }) {
  return (
    <div className="pp-snack" role="status" style={style}>
      <Icon name="check" size={18} stroke={2.25} color="var(--pp-primary-300)" />
      <span style={{ flex: 1 }}>{children}</span>
      {secondary && <button type="button" className="pp-snack-secondary pp-interactive">{secondary}</button>}
      {action && <button type="button" className="pp-snack-action pp-interactive" onClick={onAction}>{action}</button>}
    </div>
  );
}
...
function SectionHeader({ children, action, onAction }) {
  return (
    <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '4px 4px 8px' }}>
      ...
      {action && (
        <button type="button" className="pp-interactive" onClick={onAction} style={{
          background: 'none', border: 0, cursor: 'pointer',
          fontFamily: 'var(--pp-font)', fontSize: 13, lineHeight: '18px',
          color: 'var(--pp-text-link)', padding: '13px 8px', margin: '-13px -8px',
        }}>{action}</button>
      )}
    </div>
  );
...
function ActivityStrip({ children, pulse }) {
  return (
    <div className="pp-strip" style={{
      ...
      boxShadow: pulse ? '0 0 0 2px rgba(199,127,79,0.16)' : 'none',
    }}>
...
Object.assign(window, {
  Phone, StatusBar, Button, TextLink, NavbarAction, Pill, Card, HeroCard,
