      ...
      <div style={{ flex: 1, overflow: 'auto', padding: '8px 16px 24px' }}>
        ...
        <Card padding={0}>
          ...
          <div style={{ padding: 14 }}>
            ...
            }}>{filled ? "Mild post-visit note, resolved within 24 hours." : "Notes are visible only to you and invited family members."}</div>
    ...
    <Phone>
      ...
      <div style={{ flex: 1, overflow: 'auto', padding: '8px 16px 24px' }}>
        <div style={{ display: 'flex', alignItems: 'flex-start', gap: 14, marginBottom: 8 }}>
          ...
          <div style={{ flex: 1 }}>
            ...
            <div style={{ marginTop: 8 }}>
              ...
              {pill === 'review' && <Pill tone="needs-vet-review">Ask your vet</Pill>}
            </div>
          </div>
        </div>
        ...
        <Card padding={0}>
          ...
          {/* Status value is a noun («Needs vet review»). The CTA «Ask your vet»
             belongs in the pill, not as the Status field value. Review pass 3, P2. */}
          <DetailRow label="Status" value={pill === 'confirmed' ? 'Confirmed' : 'Needs vet review'} />
          <DetailRow label="Clinic" value="Example Vet Clinic" />
          <DetailRow label="Note" value="Mild post-visit note, resolved within 24 hours." multiline />
        </Card>
...
// StageStrip — progress indicator with a SINGLE active stage. Earlier draft
// underlined the first N segments, which read as tabs (review pass 3, P2).
// Active = teal-filled; past = muted-tint; future = empty.
    ...
    <div>
      <div style={{ display: 'flex', gap: 6, marginBottom: 10 }}>
        {stages.map((s, i) => {
          ...
          const bg = active ? 'var(--pp-primary-500)' : past ? 'var(--pp-primary-200)' : 'var(--pp-surface-sunken)';
          return <div key={s} style={{ flex: 1, height: 6, borderRadius: 999, background: bg }} />;
        })}
      </div>
      <div className="pp-footnote" style={{ color: 'var(--pp-text-secondary)' }}>
        Now: <b style={{ color: 'var(--pp-text-primary)' }}>{stages[current]}</b>
        {current < stages.length - 1 && <> · Next: {stages[current + 1]}</>}
      </div>
    </div>
  );
}
