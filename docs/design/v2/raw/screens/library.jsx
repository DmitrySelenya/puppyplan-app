    ...
    <div className="pp-app" style={{
      ...
      <LibraryRow title="Surface" anno="color.surface.*">
        <div style={{ display: 'flex', gap: 12, flexWrap: 'wrap' }}>
          <ColorSwatch name="base" hex="#F6EFE3" />
          <ColorSwatch name="raised" hex="#FFFCF6" />
          <ColorSwatch name="sunken" hex="#ECE3D4" />
          <ColorSwatch name="scrim 34%" hex="rgba(40,30,22,0.34)" value="rgba(40,30,22,0.34)" />
        </div>
      </LibraryRow>

      <LibraryRow title="Text" anno="color.text.*">
        <div style={{ display: 'flex', gap: 28, flexWrap: 'wrap', alignItems: 'baseline' }}>
          <div><div className="pp-body" style={{ color: 'var(--pp-text-primary)' }}>Primary</div><div className="pp-anno">#2C2824</div></div>
          <div><div className="pp-body" style={{ color: 'var(--pp-text-secondary)' }}>Secondary</div><div className="pp-anno">#6B6256</div></div>
          <div><div className="pp-body" style={{ color: 'var(--pp-text-tertiary)' }}>Tertiary</div><div className="pp-anno">#877B68</div></div>
          <div><div className="pp-body" style={{ color: 'var(--pp-text-link)' }}>Link</div><div className="pp-anno">#B26A3C</div></div>
        </div>
      </LibraryRow>

      <LibraryRow title="Primary — Clay" anno="color.primary.50–800">
      ...
      <LibraryRow title="Support — Sage · wellbeing / calm" anno="color.sage.*">
        <div style={{ display: 'flex', gap: 8 }}>
          {[['100','#E8EEDD'],['300','#BCD0A0'],['500','#84A06A'],['700','#5E7A3E']].map(([k, h]) => (
            <ColorSwatch key={k} name={`sage ${k}`} hex={h} value={h} />
          ))}
        </div>
      </LibraryRow>

      <LibraryRow title="Accent — Honey · celebration only" anno="color.accent.* — never default CTA">
        <div style={{ display: 'flex', gap: 8 }}>
          {[['100','#FBEFD9'],['300','#F2D196'],['500','#E3A53C'],['700','#B07A1E']].map(([k, h]) => (
      ...
      <LibraryRow title="Status — muted only · danger user-marked only" anno="color.status.*">
        <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
          <ColorSwatch name="success" hex="#5C7A45" value="#5C7A45" />
          <ColorSwatch name="warning" hex="#A87A2A" value="#A87A2A" />
          <ColorSwatch name="danger" hex="#A24A3C" value="#A24A3C" />
          <ColorSwatch name="info" hex="#6E5862" value="#6E5862" />
        </div>
      </LibraryRow>

      {/* TYPE */}
      <LibraryRow title="Type scale" anno="Quicksand · Nunito · tabular numerals">
      ...
      <LibraryRow title="Radius" anno="radius.{sm,md,lg,full}">
        <div style={{ display: 'flex', gap: 16 }}>
          {[['sm', 10], ['md', 14], ['lg', 20], ['full', 999]].map(([k, r]) => (
            <div key={k} style={{ textAlign: 'center' }}>
      ...
      <div style={{ marginTop: 8 }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
          {[
            ...
            ['Type', 'Quicksand (display) + Nunito (text) · warm & rounded'],
          ].map(([k, v], i) => (
