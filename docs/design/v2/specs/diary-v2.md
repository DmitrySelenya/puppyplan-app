...
3. Scroll content (horizontal padding `16`, bottom inset `tokens.layout.bottomInsetFab`):
   - **InfoHero** (optional guidance) — `info-tint` bg, `info.circle` @24 in `status/info`, `callout` body in text/primary. Radius `20`, padding `15/16`, margin-bottom `18`. **Guidance text only — no primary CTA.** An earlier code drift rendered a hero "Начать"/"Start" button that just opened Quick Log, duplicating the central `+` FAB; it was removed (owner call, live dogfood 2026-07-21) so the `+` FAB stays the single add entry point (see memory `diary-single-entry-point`).
   - **Section title "Today"** — `title-3`, padding-bottom `12`.
   - **Event list** — vertical, gap `10`, chronological. Two row types:
     - **RoutineCard** (a plan/checkable item). Left **TimeGutter** (62pt per the telegram-parity no-truncation lock, left-aligned per PUP-37 owner decision 2026-07-20; time in Lora 13.5/700 + meridiem 10/700, text/secondary). Card radius `18`, padding `10 7 10 13`, gap `9`, contents: **CheckCircle** (44pt hit, 28pt circle) → **IconChip** (44pt, radius 13, accent bg/fg) → title (`headline`) + optional meta (`footnote`) + optional "Notifications off" row (`ui.bell.slash` @13 + text) → **OverflowButton** (44pt `more.h`). States:
