...
|---|---|---|
| **Аня, solo first-time owner** | плохо спит, перегружена, не потерпит долгий setup | onboarding ≤45 сек, anonymous auth, first-log до account wall |
| **Марк и Оля, shared care** | боятся duplicate feeding/walks | last-action attribution, duplicate warning, household activity strip |
| **Лена, owner + trainer** | хочет показать прогресс без раскрытия всего household | trainer_viewer role, scope selector с included/excluded preview |
...
```
+----------------------------------------------+
|  Луна · 8 нед             [Оля][Ты]          |  <- TopBar 56pt
|  +----------------------------------------+  |
|  | Оля покормила Луну · 42 мин назад      |  |  <- ActivityStrip 36pt
|  +----------------------------------------+  |
|                                              |
...
**Accessibility.**
- TopBar avatars: «Участники: Оля, вы.»
...
```
  ...
  | Луна спала 6 часов — мочевой полный.   |
  | [ Логировать выход ]                   |
  +----------------------------------------+
```
...
- Hero: «Утренний выход» + «Луна спала 6 часов — мочевой полный.»
...
**Копия.**
- Single: «Оля покормила Луну · 42 мин назад»
- Multiple: «Оля и вы · 3 события за последний час»
...
```
  ...
  |                                        |
  | Стало понятнее: Луна обычно просится   |
...
**Live state.** При активном таймере в Today появляется persistent banner: «Луна спит · 0:24 · [Остановить]».

**Копия.**
- «Начать сейчас» / «Уже спал»
- Live banner: «Луна спит · MM:SS» + «Остановить»
...
- Actor attribution: «Записано: Оля» / «Записано: Вы».
...
|         присоединиться к заботе о Бублике                |
|                                                          |
|  Ваша роль: Опекун                                       |
|                                                          |
|  +----------------------------------------------------+  |
...
- *already-member* — «Вы уже в household Бублика.» + кнопка «Открыть».
...
- *error* — banner.
- *revoked-just-now* — toast «Доступ для Оли закрыт.»

**Копия.**
- Бейджи: «Владелец», «Опекун», «Наблюдатель», «Ожидает», «Истекло», «Отозвано»
- Overflow для активного участника: «Сменить роль», «Закрыть доступ»
- Confirm на «Закрыть доступ»: title «Закрыть доступ для Оли?», body «Оля больше не увидит данные {имя}. Это можно отменить, отправив новое приглашение.», destructive «Закрыть доступ», cancel «Отмена»
...
|                                                          |
|  Оля отметила кормление 4 минуты назад.                  |
|                                                          |
|  +----------------------------------------------------+  |
|  | Кормление  •  12:24  •  Оля                        |  |
|  +----------------------------------------------------+  |
...
- Feeding: «Оля покормила {имя} — 12 мин назад»
- Potty: «Оля отметила туалет — 4 мин назад»
- Walk: «Дмитрий вернулся с прогулки — 1 ч назад»
- Reminder created: «Оля добавила напоминание — только что»
- Note: «Дмитрий оставил заметку — 30 мин назад»
- Health log: «Дмитрий записал визит к ветеринару — 2 ч назад»
- Sitter completion: «Оля (ситтер): {имя} накормлен — 6 мин назад»
...
|  Оля будет видеть чеклист и получать напоминания.        |
|                                                          |
|  Кто                                                     |
|  +----------------------------------------------------+  |
|  | Оля                            Опекун              |  |
...
- *single caregiver, pending invite* — disabled primary + подсказка «Ждём, пока Оля примет приглашение.»
- *ready* — основной.
...
|                                                          |
|  Сделано                                                 |
|  [x] Прогулка 13:00 — Оля                                |
|  [x] Кормление 12:00 — Оля                               |
|                                                          |
|  [ Открыть таймлайн ]                                    |
+----------------------------------------------------------+
```
...
|  Оля • до 19 мая, 09:00                                  |
|                                                          |
|  Последнее: Оля накормила {имя} — 6 мин назад            |
|                                                          |
...
**Состояния.**
- *active, no events yet* — «Пока без отметок. Оля начнёт скоро.»
...
|                                                          |
|  Оля останется опекуном и сможет отмечать события,       |
...
**Состояния.**
- completed: «Готово · 19:24 · Аня». Push «trusted-sitter completion» уходит владельцу.
