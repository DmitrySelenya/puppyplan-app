...
**i18n contract.** MVP copy ships in English, Russian, and Spanish (`STRINGS.en.json`, `STRINGS.ru.json`, `STRINGS.es.json`). Design examples may stay in Russian for this source document, but native implementation must use typed i18n keys and run EN/RU/ES parity plus string-budget checks. Any new design example must map to a typed i18n key before implementation handoff so EN/RU/ES surfaces exist downstream.
...
2. **Логировать одной рукой.** Quick Log FAB всегда в thumb zone. Обычный одношаговый факт из любого экрана сохраняется максимум за 3 тапа: `Add → Quick Log → tracker`. Potty/Sleep с обязательным выбором результата могут потребовать четвёртый подтверждающий тап.
...
|---|---|---|
| **Владелец A, solo first-time owner** | плохо спит, перегружен, не потерпит долгий setup | onboarding ≤45 сек, anonymous auth, first-log до account wall |
...
2. **Fast Thumb Logging.** Обычный одношаговый Quick Log должен быть выполнен одной рукой максимум за 3 тапа из любого экрана: `Add → Quick Log → tracker`. Tracker Tiles и FAB живут в нижней трети экрана, минимальный target 56×56pt. Potty/Sleep не скрывают обязательный выбор результата и поэтому могут занимать четвёртый тап.
...
|  +----------------------------------------+  |
|  | Опекун A: кормление · 42 мин назад     |  |  <- ActivityStrip 36pt
|  +----------------------------------------+  |
|                                              |
...
```
  +----------------------------------------+
  | [avatar Опекун A] Опекун A покормил    |
  | Щенка A ·                              |
  | 42 мин назад                           |
  +----------------------------------------+
```
...
**Копия.**
- Single: «Опекун A покормил Щенка A · 42 мин назад»
...
```
  ...
  |                                        |
  | Стало понятнее: Щенок A просится       |
...
**Цель.** Залогировать обычный одношаговый факт максимум за 3 тапа из любого экрана (`Add → Quick Log → tracker`). Если тип требует явного результата (Potty/Sleep), допускается четвёртый подтверждающий тап.
...
```
  ...
  |  ---- (grabber)                         |
  |                                         |
  |  Другой опекун уже логировал кормление  |
  |  менее минуты назад.                    |
  |  Всё равно добавить?                    |
  |                                         |
...
**Копия.**
- Title: «Другой опекун уже логировал {tracker} менее минуты назад.»
...
- Timestamp: friendly bucket. «только что», «10 минут назад», «12:40», «вчера 18:15».
...
|         присоединиться к заботе о Щенке A                |
|                                                          |
|  Ваша роль: Опекун                                       |
|                                                          |
|  +----------------------------------------------------+  |
...
- *already-member* — «Вы уже в household Щенка A.» + кнопка «Открыть».
...
**Копия.**
- «Кажется, это уже отмечено»
- «Другой опекун отметил {тип события} менее минуты назад.»
...
- Feeding: «Опекун A покормил {имя} — 15 мин назад»
- Potty: «Опекун A отметил туалет — 4 мин назад»
- Walk: «Опекун B вернулся с прогулки — 1 ч назад»
- Reminder created: «Опекун A добавил напоминание — только что»
- Note: «Опекун B оставил заметку — 30 мин назад»
- Health log: «Опекун B записал визит к ветеринару — 2 ч назад»
...
|                                                          |
|  Последнее: Опекун A покормил {имя} — 6 мин назад        |
|                                                          |
...
**Состояния.**
- completed: «Готово · 19:24 · Ситтер A». Push «trusted-sitter completion» уходит владельцу.
...
- All user-facing copy maps to typed EN/RU/ES keys; examples may remain Russian
...
- All copy maps to typed i18n keys for EN/RU/ES
...
- [ ] Copy mapped to typed EN/RU/ES keys; Russian examples are allowed in this source doc
...
- [ ] Time formatting: «10 минут назад» / «Вчера, 21:10» / «12 мая, 09:42»
