ALTER TABLE public.invite
  ADD CONSTRAINT invite_email_hash_format
  CHECK (email_hash IS NULL OR email_hash ~ '^sha256:[A-Za-z0-9._:-]+$');

ALTER TABLE app_private.invite_secret
  ADD CONSTRAINT invite_secret_token_hash_format
  CHECK (token_hash ~ '^(argon2id:|\$argon2id\$).+');

ALTER TABLE app_private.share_link_secret
  ADD CONSTRAINT share_link_secret_token_hash_format
  CHECK (token_hash ~ '^(argon2id:|\$argon2id\$).+');

ALTER TABLE public.subscription_entitlement
  ADD CONSTRAINT subscription_entitlement_provider_customer_hash_format
  CHECK (
    provider_customer_id_hash IS NULL
    OR provider_customer_id_hash ~ '^sha256:[A-Za-z0-9._:-]+$'
  );
...
CREATE OR REPLACE VIEW public.share_routine_summary
WITH (security_barrier = true, security_invoker = true)
AS
SELECT * FROM public.current_share_routine_summary();

CREATE OR REPLACE VIEW public.share_selected_timeline
WITH (security_barrier = true, security_invoker = true)
AS
SELECT * FROM public.current_share_selected_timeline();

CREATE OR REPLACE VIEW public.share_training_notes
WITH (security_barrier = true, security_invoker = true)
AS
SELECT * FROM public.current_share_training_notes();

CREATE OR REPLACE VIEW public.share_health_summary
WITH (security_barrier = true, security_invoker = true)
AS
SELECT * FROM public.current_share_health_summary();

CREATE OR REPLACE VIEW public.share_puppy_profile
WITH (security_barrier = true, security_invoker = true)
AS
SELECT * FROM public.current_share_puppy_profile();
